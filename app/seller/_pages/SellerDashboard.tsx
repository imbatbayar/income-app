...
/**
 * Seller dashboard-ийг route/page бүр дээр дахин ашиглахад зориулсан wrapper.
 * UI-г хөндөхгүйгээр логикийг нэг газар төвлөрүүлж байна.
 */
const TAB_TO_ROUTE: Record<SellerTabId, string> = {
  OPEN: "/seller/open",
  ASSIGNED: "/seller/chosen",
  ON_ROUTE: "/seller/onroad",
  DELIVERED: "/seller/delivered",
};

export default function SellerDashboard(props: {
  initialTab?: SellerTabId;
  routeMode?: boolean;
}) {
  ...
  const [activeTab, setActiveTab] = useState<SellerTabId>(
    props.initialTab ?? "OPEN"
  );
  ...
  }, [sp, SELLER_TABS_UI, props.initialTab]);

  function changeTab(tab: SellerTabId) {
    setActiveTab(tab);
    localStorage.setItem("sellerActiveTab", tab);
    if (props.routeMode) {
      router.replace(TAB_TO_ROUTE[tab] || "/seller/open");
      return;
    }
