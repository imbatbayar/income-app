...
const TAB_TO_ROUTE: Record<DriverTabId, string> = {
  OFFERS: "/driver/open",
  PICKUP: "/driver/chosen",
  IN_TRANSIT: "/driver/onroad",
  DONE: "/driver/delivered",
};
...
function DriverPageInner(props: { initialTab?: DriverTabId; routeMode?: boolean }) {
  const router = useRouter();
  const sp = useSearchParams();

  const [user, setUser] = useState<IncomeUser | null>(null);
  const [activeTab, setActiveTab] = useState<DriverTabId>(props.initialTab ?? "OFFERS");
  ...
  function changeTab(tab: DriverTabId) {
    setActiveTab(tab);
    if (props.routeMode) {
      router.push(TAB_TO_ROUTE[tab] || "/driver/open");
      return;
    }
